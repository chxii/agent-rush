...
**为什么是 GLM / Z.AI**：长程任务能力正是 GLM 这代模型的强项——稳定的多步规划、可靠的结构化（JSON）输出、以及流式推理。我把它放在游戏里**唯一不可替代**的位置（Executor 的执行循环），而不是到处撒 LLM 调味。具体对齐 Z.AI 能力的三点：
...
**现实性诚实声明**：游戏建模的是 2019–2021 年经典 MEV（公开 mempool + Gas 公开竞价，即大众心智模型）。当下主流已转向 Flashbots / PBS / 私有订单流，竞价搬进了 builder 的"密封小黑屋"，且失败 bundle 不上链、不烧 gas（与游戏"失败烧 Gas"相反）。选用旧模型是**自觉的教学简化**——它最快让人懂 MEV，且可见的对抗才好玩。规则页对此有明确标注。详见 GDD §14。
...
- **实机测试**：真人试玩返工（教学、UI、干预、平衡），结论合并 main，线上版 v1.0.0 上线。
