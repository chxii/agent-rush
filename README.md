...
游戏里有四种 Agent，但只有 **Executor 是真实的大模型调用**，其余三个是规则引擎（信息处理类任务，规则即可胜任）。这是刻意的设计——把 LLM 用在它真正不可替代的地方：

| Agent | 阶段 | 职责 | 实现 |
|-------|------|------|------|
| **Searcher** | 扫描 | 扫描机会、生成手牌 | 规则引擎 |
| **RiskAnalyzer** | 扫描 | 评估每张牌风险、识别骗局牌 | 规则引擎 |
| **Strategist** | 扫描 | 推荐最优出牌组合 | 规则引擎 |
| **Executor** | 执行 | **动态执行、自动调度 Gas、失败恢复、跨目标重规划** | **真实 GLM 调用** |

Executor 体现的正是 Long-Horizon 能力的五个特征，每个都对应游戏里一个**真实的 GLM 调用点**，并由 JSON Schema 严格约束输出：

1. **任务分解**（InitialPlanning）— 把"执行 N 张牌"拆解、排序、分配 Gas
2. **多步规划**（SingleCardPlan）— 把单张牌拆成具体的链上动作序列
3. **工具调用**（ExecutionStep）— `fetch_prices` / `broadcast_tx` / `replace_tx` / `monitor_mempool` 等
4. **迭代修复**（IncidentResponse）— 目标被对手抢占时，实时评估"提高 Gas 压制"还是"放弃止损"
5. **工作流闭环**（SettlementReport）— 复盘本轮、生成结算报告与决策高亮
...
```
扫描 (Scan)          出牌 (Play)           执行 (Execute)          结算 (Settle)
Searcher 生成手牌  →  选牌 + 配 Gas      →  刚体模式 / 自适应模式  →  损益结算 + 进度推进
RiskAnalyzer 标风险   时间窗口内决策          ↓
Strategist 给推荐                          有 Executor → 自适应模式
                                          （Thought Chain 流式展示）
```

### 两种执行模式

- **刚体模式**（未解锁 Executor）：牌按固定顺序、固定 Gas 执行，失败无补救。这是对照组——让玩家先体会"没有 Agent 的世界"。
- **自适应模式**（解锁 Executor 后，层 8 起）：Executor 接管执行，实时拆解、调度、修复。这是主角，Long-Horizon 能力在此完整展现。

### 20 关进程

| 层段    | 对手 Bot      | 场景       | Agent 槽位 | 关键节点                          |
| ----- | ----------- | -------- | -------- | ----------------------------- |
| 1-3   | 无 / Bot-404 | DEX 套利   | 1        | 教程：零门槛上手                      |
| 4     | Bot-404     | DEX 套利   | 2        | **Boss**，解锁 RiskAnalyzer      |
| 5-7   | Shadow      | +新币发射台   | 2        | 引入场景选择                        |
| 8     | Phantom     | NFT 市场   | 3        | **Boss**，解锁 Executor（自适应模式开启） |
| 9-12  | Phantom     | NFT / 借贷 | 3        | Long-Horizon 完整展示             |
| 13    | Phantom+    | 借贷清算     | 3        | **Boss**，解锁 Strategist        |
| 14-17 | Phantom+    | 多场景      | 3        | 困难                            |
| 18-20 | Genesis     | 全场景      | 3        | Genesis 学习机制，预判博弈             |
...
- **第一幕（2 min）· 层 1-3**：展示零门槛上手——看牌、选牌、出牌、看结算。
- **第二幕（3 min）· 层 10**：核心展示。带 Executor 进入自适应模式，Thought Chain 流式显示 AI 推理；预设 Phantom 抢占目标，触发 **IncidentResponse 迭代修复**路径；结算面板高亮决策时刻。
...
- **接口一致性**：`ExecutorMock` 与 `ExecutorAI` 接口完全一致，调用方无需感知真实/模拟差异；由是否存在 API Key 自动切换。
- **Schema 校验**：5 个调用点的输入/输出均由 JSON Schema（ajv）约束，故意传入缺字段的 JSON 会触发 fallback 而非崩溃。
...
- **20 关进程**：层 4/8/13 的 Agent 解锁、Boss 奖励、场景选择、Genesis 学习（连续两轮同场景同开局 → 第三轮胜率 +20%）逻辑均经核对。

> 完整的开发阶段计划与各阶段验收清单见 `docs/DevPlan.md`，游戏设计细节见 `docs/GDD_AgentRush_v4.md`。
